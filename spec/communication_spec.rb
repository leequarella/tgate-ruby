...
describe Tgate::Communication, vcr: true do
  before :each do
    @xml = File.read("spec/support/sending.xml")
    @url = 'https://demo.myvirtualmerchant.com/TgateDemo/processxml.do'
    @http_referer = 'https://somereferer.com'
    @sender = Tgate::Communication.new({xml: @xml, url: @url, http_referer: @http_referer})
  end
  ...
  describe "talking to VM", vcr: true do
    ...
    context "failed communication" do
      it "returns false" do
        bad_url = "junk_url"
        sender = Tgate::Communication.new({xml: @xml, url: bad_url, http_referer: {}})
