...
describe Tgate::Response do
  it 'initializes from a happy approval xml string' do
    response = Tgate::Response.new(approval_xml)
  ...
  it "initializes from an unhappy approval xml string" do
    response = Tgate::Response.new(bad_approval_xml)
  ...
  it "initializes from an error xml string" do
    response = Tgate::Response.new(error_xml)
    response.approved.should be_false
  ...
  it "initializes from a 'false' param" do
    response = Tgate::Response.new(false)
    response.approved.should be_false
    response.result_message.should eq "Tgate did not respond."
    response.error.should eq "-1"
  end
end
